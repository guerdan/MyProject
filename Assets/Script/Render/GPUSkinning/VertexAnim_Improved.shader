...
Shader "Mobile-Game/VertexAnim_Improved"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 _MainTex_ST;
			sampler2D _AnimTex;
			float4 _AnimTex_TexelSize;//(uv里一格宽度，uv里一格高度,每行有多少格,0)
			...
			{
				float new_y = total / _AnimTex_TexelSize.z;   //行序
				float new_x = floor(fmod(new_y, 1.0) * _AnimTex_TexelSize.z);  //列序
				new_y = floor(new_y);
			...
			v2f vert (appdata v, uint vid : SV_VertexID)
			{
				...
				y = floor(y - floor(y / _FrameCount) * _FrameCount);  //当前帧序

#if _ACCURACY__1
				float total = y * _VertexCount + vid;  //取点的总序
				float2 newUv = uvConvert(total);   //转uv坐标
				float2 animUv = float2((newUv.x + 0.5) * _AnimTex_TexelSize.x, (newUv.y + 0.5) * _AnimTex_TexelSize.y);
				//允许你通过 float4 参数中的第四个分量来手动指定 LOD 级别。
