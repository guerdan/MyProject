...
{
    public class InputTextSVItemData
    {
        public string Title;
        public string Content;
        public Action EditTip;

        public InputTextSVItemData(string title, string content)
        {
            Title = title;
            Content = content;
        }
        ...
        // 执行公式的格式化
        public string DoFormulaFormat()
        {
            var raw = Content.Replace(" ", "");
            Content =  AutoDataUIConfig.FormulaFormat(raw);
            return raw;
        }
    }

    /// <summary>
    /// InputTextScrollView. 关闭时保存
    /// </summary>
    public class InputTextSV : MonoBehaviour
    {
        ...
        List<InputTextSVItemData> _strList;
        Action<List<InputTextSVItemData>> _onClose;
        float _itemWidth;
        ...
        {
            var comp = item.GetComponent<InputTextSVItem>();
            comp.SetData(_strList[index], index, _itemWidth, this);
        }

        /// <summary>
        /// 不传onSelect代表不用选项。
        /// </summary>
        public void SetData(List<InputTextSVItemData> strList, Action<List<InputTextSVItemData>> onClose
                            , float width = 160
            , int maxShowCount = 5)
        {
            _strList = strList;
            ...
            float height = oneH * count + 16 + (count - 1) * 4;
